...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False,
        ...
        self.layer5 = self._make_layer(block, 512, 2, stride=2, dilate=False)
...
class pre_module(nn.Module):
    ...
    def forward(self, x):
        ...
        inner0 = F.avg_pool2d(inner0, (32, 32), stride = 32)
        outer0 = F.avg_pool2d(outer0, (32, 32), stride = 32)

        return inner0, outer0
...
class theModel(nn.Module):
    ...
    def __init__(self, backbone):
        ...
        tmp = self.conv1.state_dict()
        tmp['weight'][:, 0:3, :, :] = resnet.conv1.weight.data
        #tmp['weight'][:, 3:, :, :] = 0
        self.conv1.load_state_dict(tmp)
        ...
        self.decoder_6 = nn.Sequential(
                ...
                nn.PReLU(decoder_channels_5),
                nn.Conv2d(decoder_channels_5, decoder_channels_5, 3, 1, 1, bias = False),
                nn.BatchNorm2d(decoder_channels_5),
                )

        self.decoder_5 = decoder_module(512, decoder_channels_5, decoder_channels_4)
        ...
        self.skip_0 = nn.Sequential(
                nn.Conv2d(3, decoder_channels_0, 3, 1, 1, bias = False),
                nn.BatchNorm2d(decoder_channels_0),
                nn.PReLU(decoder_channels_0),
                nn.Conv2d(decoder_channels_0, decoder_channels_0, 3, 1, 1, bias = False),
                nn.BatchNorm2d(decoder_channels_0),
        ...
        self.trimap_conv = nn.Sequential(
                nn.Conv2d(514, 512, 3, 1, 1, bias = False),
                nn.BatchNorm2d(512),
                )
    ...
    def forward(self, x):
        ...
        x5 = self.trimap_conv(torch.cat([x5, t1, t2], 1))

        x6 = self.relu(x5)
        x6 = self.layer5(x6)

        x6_decoder = self.decoder_6(x6)
        x5_decoder = F.interpolate(x6_decoder, x5.shape[2:], mode = "bilinear")
        x5_decoder = self.decoder_5(x5, x5_decoder)
        ...
        pred_alpha = self.decoder_0(self.skip_0(x[:,0:3,:,:]) + x0_decoder)
