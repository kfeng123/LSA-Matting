...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, withReLU = True, dilate = 1):
        super(Bottleneck, self).__init__()
        self.withReLU = withReLU
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, dilation = dilate,
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, inChannel = 4):
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], dilate = 2)
        self.layer4 = self._make_layer(block, 512, layers[3], dilate = 4)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate = 1):
        ...
        for i in range(1, blocks):
            if i!= blocks-1:
                layers.append(block(self.inplanes, planes, dilate = dilate))
            else:
                layers.append(block(self.inplanes, planes, withReLU = False, dilate = dilate))
