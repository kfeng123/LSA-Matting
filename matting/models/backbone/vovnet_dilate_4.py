...
def conv3x3(
    in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=3, padding=1, dilate = 1
):
    """3x3 convolution with padding"""
    padding = dilate
    return [
...
class _OSA_module(nn.Module):
    def __init__(
        self, in_ch, stage_ch, concat_ch, layer_per_block, module_name, SE=False, identity=False, depthwise=False, dilate = 1
    ):
        ...
        for i in range(layer_per_block):
            self.layers.append(
                nn.Sequential(OrderedDict(conv3x3(in_channel, stage_ch, module_name, i, dilate = dilate)))
            )
            in_channel = stage_ch
...
class _OSA_stage(nn.Sequential):
    def __init__(
        ...
        if not (stage_num == 2 or stage_num == 5):
        ...
        self.add_module(
            module_name, _OSA_module(in_ch, stage_ch, concat_ch, layer_per_block, module_name, SE, depthwise=depthwise, dilate = dilate)
        )
        for i in range(block_per_stage - 1):
            ...
            self.add_module(
                module_name,
                _OSA_module(
                    concat_ch, stage_ch, concat_ch, layer_per_block, module_name, SE, identity=True, depthwise=depthwise, dilate = dilate
                ),
            )
...
class VoVNet(nn.Module):
    def __init__(self, input_ch, myArch, lastStage = 5):
        ...
        self._out_feature_channels['stage4'] = self._out_feature_channels['stage5']
        del self._out_feature_channels['stage5']
    ...
    def forward(self, x):
        ...
        for name in self.stage_names:
            ...
            if name == "stage4": continue
            if name == "stage5": name = "stage4"
            outputs[name] = x
