...
class combine345(nn.Module):
    def __init__(self, in_feature_channels):
        super(combine345, self).__init__()

        self.decoder_5 = nn.Sequential(
                OrderedDict([
                    ("prelu", nn.PReLU(in_feature_channels['stage5'])),
                    ("conv", nn.Conv2d(in_feature_channels['stage5'], 64, 3, 1, 1, bias = False)),
        ...
        self.decoder_4 = nn.Sequential(
                OrderedDict([
                    ("prelu", nn.PReLU(in_feature_channels['stage4'])),
                    ("conv", nn.Conv2d(in_feature_channels['stage4'], 64, 3, 1, 1, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                ])
                )
        ...
        self.decoder_3 = nn.Sequential(
                OrderedDict([
                    ("prelu", nn.PReLU(in_feature_channels['stage3'])),
                    ("conv", nn.Conv2d(in_feature_channels['stage3'], 64, 3, 1, 1, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                ])
                )
        ...
        self.dilation1 = nn.Sequential(
                OrderedDict([
                    ("conv", nn.Conv2d(64 * 3, 64, kernel_size = 3, stride = 1, padding = 1, dilation=1, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                    ("prelu", nn.PReLU(64)),
                ])
                )
        ...
        self.dilation2 = nn.Sequential(
                OrderedDict([
                    ("conv", nn.Conv2d(64 * 3, 64, kernel_size = 3, stride = 1, padding = 2, dilation=2, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                    ("prelu", nn.PReLU(64)),
                ])
                )
        ...
        self.dilation4 = nn.Sequential(
                OrderedDict([
                    ("conv", nn.Conv2d(64 * 3, 64, kernel_size = 3, stride = 1, padding = 4, dilation=4, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                    ("prelu", nn.PReLU(64)),
                ])
                )

        self.dilation8 = nn.Sequential(
                OrderedDict([
                    ("conv", nn.Conv2d(64 * 3, 64, kernel_size = 3, stride = 1, padding = 8, dilation=8, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                    ("prelu", nn.PReLU(64)),
                ])
                )
        ...
        self.fuse = nn.Sequential(
                OrderedDict([
                    ("conv", nn.Conv2d(64 * 4, 64, 3, 1, 1, bias = False)),
                    ("norm", nn.BatchNorm2d(64)),
                ])
                )
    ...
    def forward(self, x3, x4, x5):
        _, _, h, w = x3.shape
        fea = [self.decoder_3(x3),
                F.interpolate(self.decoder_4(x4), (h, w), mode = "nearest"),
                F.interpolate(self.decoder_5(x5), (h,w), mode = "nearest")]
        fea = torch.cat(fea, dim = 1)
        fea = torch.cat([self.dilation1(fea), self.dilation2(fea), self.dilation4(fea), self.dilation8(fea)], dim = 1)
        fea = self.fuse(fea)
        return fea
...
class theModel(nn.Module):
    ...
    def __init__(self, myArch = "V-19-eSE"):
        ...
        self.combine345 = combine345(self.backbone._out_feature_channels)
    ...
    def forward(self, x):
        ...
        x3_decoder = self.combine345(encoder_out['stage3'], encoder_out['stage4'], encoder_out['stage5'])
