...
class theModel(nn.Module):
    ...
    def __init__(self, myArch = "V-19-eSE"):
        ...
        self.backbone = VoVNet(3, myArch = myArch)
        ...
        self.decoder_5 = nn.Sequential(
                OrderedDict([
                    ("conv1", nn.Conv2d(self.backbone._out_feature_channels['stage5'], decoder_channels_4, 3, 1, 1, bias = False)),
                    ("norm1", nn.BatchNorm2d(decoder_channels_4)),
                    ("prelu1", nn.PReLU(decoder_channels_4)),
                    ("conv2", nn.Conv2d(decoder_channels_4, decoder_channels_4, 3, 1, 1, bias = False)),
                    ("norm2", nn.BatchNorm2d(decoder_channels_4)),
                ])
                )
        #self.decoder_5 = decoder_module(self.backbone._out_feature_channels['stage5'], decoder_channels_4, decoder_channels_3)

        self.decoder_4 = decoder_module(self.backbone._out_feature_channels['stage4'], decoder_channels_4, decoder_channels_3)
        self.decoder_3 = decoder_module(self.backbone._out_feature_channels['stage3'], decoder_channels_3, decoder_channels_2)
        self.decoder_2 = decoder_module(self.backbone._out_feature_channels['stage2'], decoder_channels_2, decoder_channels_1)
        self.decoder_1 = decoder_module(self.backbone._out_feature_channels['stage1'], decoder_channels_1, decoder_channels_0)
        ...
        self.preprocess_trimap = preprocess_trimap()
        self.trimap_fusion = trimap_fusion()

    def forward(self, x):

        trimaps = self.preprocess_trimap(x)

        encoder_out = self.backbone(x[:,:3,:,:])


        x5_decoder = self.decoder_5(encoder_out['stage5'])

        x4_decoder = F.interpolate(x5_decoder, encoder_out['stage4'].shape[2:], mode = "nearest")
        x4_decoder = self.decoder_4(encoder_out['stage4'], x4_decoder)

        x3_decoder = F.interpolate(x4_decoder, encoder_out['stage3'].shape[2:], mode = "nearest")
        x3_decoder = self.decoder_3(encoder_out['stage3'], x3_decoder)

        x3_decoder = self.trimap_fusion(trimaps, x3_decoder)

        x2_decoder = F.interpolate(x3_decoder, encoder_out['stage2'].shape[2:], mode = "nearest")
        x2_decoder = self.decoder_2(encoder_out['stage2'], x2_decoder)

        x1_decoder = F.interpolate(x2_decoder, encoder_out['stage1'].shape[2:], mode = "nearest")
        x1_decoder = self.decoder_1(encoder_out['stage1'], x1_decoder)
