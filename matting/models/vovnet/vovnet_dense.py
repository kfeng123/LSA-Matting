...
class theModel(nn.Module):
    ...
    def __init__(self, myArch = "V-19-eSE"):
        ...
        for i in range(2, 6):
            for j in range(1, i):
                self.add_module("trans1_"+str(i)+"_"+str(j),
                    nn.Sequential(
                        OrderedDict([
                            ("conv", nn.Conv2d(64, 64, 1, 1, 0, bias = False)),
        ...
        for i in range(2, 5):
            for j in range(1, i):
                self.add_module("trans2_"+str(i)+"_"+str(j),
                    nn.Sequential(
                        OrderedDict([
                            ("conv", nn.Conv2d(64, 64, 1, 1, 0, bias = False)),
                            ("norm", nn.BatchNorm2d(64)),
        ...
        for i in range(2, 4):
            for j in range(1, i):
                self.add_module("trans3_"+str(i)+"_"+str(j),
                    nn.Sequential(
                        OrderedDict([
                            ("conv", nn.Conv2d(64, 64, 1, 1, 0, bias = False)),
                            ("norm", nn.BatchNorm2d(64)),
        ...
        for i in range(2, 3):
            for j in range(1, i):
                self.add_module("trans4_"+str(i)+"_"+str(j),
                    nn.Sequential(
                        OrderedDict([
                            ("conv", nn.Conv2d(64, 64, 1, 1, 0, bias = False)),
                            ("norm", nn.BatchNorm2d(64)),
    ...
    def forward(self, x):
        ...
        for i in range(2, 6):
            for j in range(1, i):
                skip_out['stage' + str(j)] = skip_out['stage' + str(j)] + \
                F.interpolate(
                    getattr(self, "trans1_"+str(i) + "_" + str(j) )(skip_out['stage'+str(i)]),
                    skip_out['stage' + str(j)].shape[2:], mode = "nearest"
                    )
        ...
        for i in range(2, 5):
            for j in range(1, i):
                skip_out['stage' + str(j)] = skip_out['stage' + str(j)] + \
                F.interpolate(
                    getattr(self, "trans2_"+str(i) + "_" + str(j) )(skip_out['stage'+str(i)]),
                    skip_out['stage' + str(j)].shape[2:], mode = "nearest"
                    )
        ...
        for i in range(2, 4):
            for j in range(1, i):
                skip_out['stage' + str(j)] = skip_out['stage' + str(j)] + \
                F.interpolate(
                    getattr(self, "trans3_"+str(i) + "_" + str(j) )(skip_out['stage'+str(i)]),
                    skip_out['stage' + str(j)].shape[2:], mode = "nearest"
                    )
        ...
        for i in range(2, 3):
            for j in range(1, i):
                skip_out['stage' + str(j)] = skip_out['stage' + str(j)] + \
                F.interpolate(
                    getattr(self, "trans4_"+str(i) + "_" + str(j) )(skip_out['stage'+str(i)]),
                    skip_out['stage' + str(j)].shape[2:], mode = "nearest"
                    )
