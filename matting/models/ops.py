...
if __name__ == "__main__":
    ...
    model = GuidedCxtAtten(16, 16)
    model.eval()
    model.cuda()
    dump_x = torch.randn(1, 16, 256, 256).cuda()
    dump_x_2 = torch.randn(1, 16, 256, 256).cuda()
    unknown = torch.randn(1, 1, 256, 256).cuda()
    y = model(dump_x, dump_x_2, unknown)
    import pdb
    pdb.set_trace()
